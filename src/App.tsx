...
import ColorBar from "./features/ColorBar/ColorBar";
import InclinedWheel from "./features/InclinedWheel/InclinedWheel";
import CodeToCopy from "./features/CodeToCopy/CodeToCopy";
...
function App() {
  ...
  const [dynamicGradientStops, setDynamicGradientStops] = useState<
    GradientStop[]
  >([]);
  const [activeDynamicStopIndex, setActiveDynamicStopIndex] = useState<
    number | null
  >(null); // Nowy stan do śledzenia aktywnego dynamicznego przystanku
  ...
  const handleAddDynamicGradientStop = (color: string, percent: number) => {
    ...
    const existingIndex = dynamicGradientStops.findIndex(
      (stop) => stop.percent === percent,
    );
    if (existingIndex !== -1) {
      setDynamicGradientStops((prevStops) =>
        prevStops.map((stop, index) =>
      ...
      setDynamicGradientStops((prevStops) => {
        ...
        setActiveDynamicStopIndex(
          newStops.findIndex(
            (stop) => stop.color === color && stop.percent === percent,
          ),
        );
        return newStops;
      });
  ...
  const sortedDynamicGradientStops = [...dynamicGradientStops].sort(
  ...
  if (dynamicGradientStopsString) {
    // Dodaj dynamiczne przystanki tylko jeśli istnieją
    fullGradientString += `, ${dynamicGradientStopsString}`;
  }

  fullGradientString += `, ${button2Color} 100%`;

  const handleUpdateGradientStopPercent = (
    index: number,
    newPercent: number,
  ) => {
